...
describe("resolveProviderEntries", () => {
  ...
  it("default (no flags) → standard tier model", () => {
    ...
    expect(entries).toEqual([{ name: "openai", model: "gpt-image-2" }]);
  });
  ...
  it("default for gemini → gemini-3.1-flash-image", () => {
    const entries = resolveProviderEntries(
      ["gemini"],
      { ...BASE_FLAGS },
      PROVIDER_QUALITIES,
    ...
    expect(entries).toEqual([
      { name: "gemini", model: "gemini-3.1-flash-image" },
    ]);
  });
  ...
  it("--tier premium → premium models", () => {
    const entries = resolveProviderEntries(
      ["openai", "recraft", "gemini"],
      { ...BASE_FLAGS, tier: "premium" },
      PROVIDER_QUALITIES,
    );
    expect(entries).toEqual([
      // openai has no premium mapping → model omitted, provider default applies
      { name: "openai" },
      { name: "recraft", model: "recraftv4_pro" },
      { name: "gemini", model: "gemini-3-pro-image" },
    ]);
  });
  ...
  it("--tier economy → economy models", () => {
    ...
    expect(entries).toEqual([
      { name: "gemini", model: "gemini-3.1-flash-lite-image" },
    ]);
  });
  ...
  it("--tier + --model → ValidationError", () => {
    expect(() =>
      resolveProviderEntries(
        ["openai"],
        { ...BASE_FLAGS, tier: "premium", model: "gpt-image-2" },
        PROVIDER_QUALITIES,
      ),
  ...
  it("--vector on openai → standard tier (no-op)", () => {
    ...
    expect(entries[0].model).toBe("gpt-image-2"); // standard tier, vector ignored for openai
  });
  ...
  it("multiple providers with default tier", () => {
    ...
    expect(entries[0]).toEqual({ name: "openai", model: "gpt-image-2" });
