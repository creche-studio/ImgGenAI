...
describe("ProviderRegistry", () => {
  ...
  it("register + resolve returns a Provider instance", () => {
    ...
    const provider = registry.resolve(pn("mock"), { MOCK_API_KEY: "sk-test" });
  ...
  it("throws ValidationError for unknown provider with available list in hint", () => {
    ...
    expect(() => registry.resolve(pn("nope"))).toThrow(ValidationError);

    try {
      registry.resolve(pn("nope"));
    } catch (err) {
  ...
  it("throws ConfigError when API key is missing with export hint", () => {
    ...
    expect(() => registry.resolve(pn("mock"), {})).toThrow(ConfigError);

    try {
      registry.resolve(pn("mock"), {});
    } catch (err) {
  ...
  it("has() returns true for registered and false for unregistered", () => {
    ...
    expect(registry.has(pn("exists"))).toBe(true);
    expect(registry.has(pn("nope"))).toBe(false);
  });
});
